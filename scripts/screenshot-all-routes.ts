...
const SHOTS: Shot[] = [
  {
    name: "home",
    url: "/",
    description: "Landing page (TanStack Start template, branded as LATTICE)",
  },
  {
    name: "runtime",
    url: "/runtime",
    description: "Operator console — runs table + active stream",
  },
  {
    name: "runtime-timeline",
    url: "/runtime",
    description: "Operator console scrolled to EventTimeline + AgentUI panel",
    scrollToBottom: true,
  },
  { name: "runs", url: "/runs", description: "Runs index (placeholder route)" },
  {
    name: "agents",
    url: "/agents",
    description: "Agents index (route not yet scaffolded — should 404)",
  },
  {
    name: "threads",
    url: "/threads",
    description: "Threads index (placeholder route)",
  },
  {
    name: "evidence",
    url: "/evidence",
    description: "Evidence ledger (placeholder route)",
  },
  {
    name: "replay",
    url: "/replay",
    description: "Run replay (placeholder route)",
  },
  {
    name: "settings",
    url: "/settings",
    description: "Settings (placeholder route)",
  },
];
...
async function preflight(): Promise<void> {
  ...
  for (const p of probes) {
    const r = await fetch(p.url).catch(
      (e) => ({ ok: false, status: 0, _err: String(e) }) as any,
    );
    if (!("ok" in r) || !r.ok) {
      console.error(
        `✗ preflight failed: ${p.name} @ ${p.url} (status ${"status" in r ? r.status : "?"})`,
      );
      console.error(
        `  hint: start the sidecar with 'make sidecar-up-tcp' and the dev server with 'bun run dev'`,
      );
      process.exit(1);
    }
    console.log(`✓ ${p.name}`);
...
async function shot(
  ...
  console.log(
    `  ${status === 200 ? "✓" : "⚠"} ${s.name.padEnd(20)} HTTP ${status}  -> ${path}`,
  );
...
async function main(): Promise<void> {
  ...
  await writeFile(
    summary,
    JSON.stringify(
      { generatedAt: new Date().toISOString(), shots: SHOTS, results },
      null,
      2,
    ),
  );
