...
async function main(): Promise<void> {
  ...
  if (existsSync(eventsJsonl)) {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        console.warn(`skipping malformed event line: ${(err as Error).message}`)
      }
    }
  }
  ...
  if (existsSync(runYaml)) {
    events.push({
      ...
      payload: {
        ...
        status: 'completed',
        outcome_md: '',
        evidence_manifest_yaml: tryRead(join(absRunDir, 'evidence-manifest.yaml')) ?? '',
  ...
  console.log(
    ...
    (client.mode === 'uds' ? ` socket=${client.socketPath}` : ` baseUrl=${client.baseUrl}`) +
    `)`,
  )
