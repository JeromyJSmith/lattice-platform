...
function RuntimePage() {
  ...
  useEffect(() => {
    const streaming = activeRunStatus === 'pending' || activeRunStatus === 'running'
    streamStore.setState((s) => (s.isStreaming === streaming ? s : { ...s, isStreaming: streaming }))
  }, [activeRunStatus])
  ...
  return (
    <main className="page-wrap px-4 pb-8 pt-14 space-y-4">
      ...
      <p className="text-sm opacity-80">
        Agent: {runtime.selectedAgent} | Active run: {runtime.activeRunId ?? 'none'} | Status:{' '}
        {activeRunStatus ?? '—'} | Streaming: {String(stream.isStreaming)} | DB rows:{' '}
      ...
      <TaskSubmitForm
        onSubmit={async (task) => {
          ...
          runtimeStore.setState((state) => ({ ...state, activeRunId: result.runId }))
          setTimeline((prev) => [...prev, `run ${result.runId} dispatched (pending)`])
      ...
      <RunsTable
        ...
        onRowClick={(row) => {
          runtimeStore.setState((state) => ({ ...state, activeRunId: row.run_id }))
        }}
      />
