...
class AggregateHealthResponse(BaseModel):
    """Aggregate health across all sections."""
...
class SectionHealthResponse(BaseModel):
    """Per-section health breakdown for a single section."""
...
class ProposalRequest(BaseModel):
    """Body for POST /proposals — a new harness proposal."""
...
class ProposalResponse(BaseModel):
    """Response for POST /proposals — the assigned proposal id."""

    proposal_id: str


class ProposalsListResponse(BaseModel):
    """Response for GET /proposals — list of proposals plus count."""
...
class EventsListResponse(BaseModel):
    """Response for GET /events — list of section events plus count."""
...
class RatchetRequest(BaseModel):
    """Body for POST /ratchet — kick off a ratchet cycle for one section."""

    section: str = Field(..., min_length=1, max_length=64)


class RatchetResponse(BaseModel):
    """Response for POST /ratchet — the cycle id and started status."""
...
class ScoreSnapshotResponse(BaseModel):
    """Aggregate per-section score snapshot returned by GET /score."""
...
def post_proposal(
    ...
    def do() -> dict[str, Any]:
        """Insert the proposal row idempotently."""
        try:
...
async def post_ratchet(
    ...
    def do() -> dict[str, Any]:
        """Log the cycle-started event and fire the async ratchet runner."""
        _log_event()
