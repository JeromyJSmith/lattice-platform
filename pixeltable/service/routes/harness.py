...
RUNNABLE_CAPABILITY_TASKS: dict[str, dict[str, Any]] = {
    ...
    "quantity-takeoff-agent": {
        "kind": "script_exit_code",
        "action_id": "run-quantity-takeoff-agent",
        "job_id": "quantity-takeoff-agent",
        "label": "Run blocker proof",
        "endpoint": "POST /v1/harness/capabilities/runs",
        "command": [
            "uv",
            "run",
    ...
    "ddc-estimation-contract": {
        "kind": "script_exit_code",
        "action_id": "run-ddc-estimation-contract",
        "job_id": "ddc-estimation-contract",
        "label": "Run blocker proof",
        "endpoint": "POST /v1/harness/capabilities/runs",
        "command": [
            "uv",
            ...
            capabilities.append(
                {
                    ...
                    "declared_status_color": str(capability.get("status_color", "")).lower() or None,
                    ...
                    "project_target": capability.get("project_target"),
                    "proof_lineage": normalize_list(capability.get("proof_lineage")),
                    "supported_by": normalize_list(capability.get("supported_by")),
                    "blocking_capabilities": normalize_list(capability.get("blocking_capabilities")),
                    "follow_on": capability.get("follow_on"),
