...
4. **The schema** — a configuration document (AGENTS.md for Codex and AGENTS.md for Codex) that tells the LLMs how the wiki is structured, what conventions to follow, and what workflows to use. Co-evolved by user and LLM over time. Create both files so the folder is compatible with most LLMs.
...
4. SCHEMA      -> Write the AGENTS.md and AGENTS.md of the local folder where the skill is invoked for configuration
...
This is the most important phase. The schema (AGENTS.md / AGENTS.md) is what turns a generic LLM into a disciplined wiki maintainer.
...
2. **AGENTS.md / AGENTS.md** — the schema document from Phase 4
...
If this is the very first processing run, flag any adjustments needed to `AGENTS.md` / `AGENTS.md`:
...
- Compress phases 3-6. Use a minimal structure: `raw/`, `wiki/`, `AGENTS.md`
