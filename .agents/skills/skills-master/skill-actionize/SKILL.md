...
This gives the user three commands they can run from the terminal without Codex:
...
**IMPORTANT:** Do NOT use CronCreate here — it is session-scoped and dies when Codex
...
Codex. Output logs to `.plan/cron.log`. On macOS, if reminders don't arrive,
...
To show overdue + today's tasks when Codex opens in this project, add a
reminder note to AGENTS.md.

Check if AGENTS.md already has an actionize reminder:

```bash
grep -q "actionize" AGENTS.md 2>/dev/null && echo "ALREADY_CONFIGURED" || echo "NOT_CONFIGURED"
```

If NOT_CONFIGURED, append to AGENTS.md:
...
- Full review via Codex:         /actionize
