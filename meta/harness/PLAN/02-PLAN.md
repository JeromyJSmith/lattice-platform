...
| 2 | Migrations 0014 (harness) + 0015 (knowledge substrate) | `pixeltable/migrations/0014_harness_schema.py`, `pixeltable/migrations/0015_knowledge_substrate.py`, `_helpers.py` adds `lattice/harness` + `lattice/knowledge` to `OWNED_PARENTS`, `meta/SCHEMA.md` + `meta/ARCHITECTURE.md` + root `CLAUDE.md` count bump 13→15, 36→45 (40 from 0014 + 5 from 0015) | `make migrate-dryrun` clean; `make migrate` applied; `pxt.list_tables('lattice/harness')` returns 4; `pxt.list_tables('lattice/knowledge')` returns 5; `uv run python -c 'import pixeltable.knowledge.tools'` succeeds |
