---
description: Drive the LATTICE ratchet loop — score a section, call claude -p to generate a unified diff proposal, apply it in a sandbox worktree, gate on score improvement, and log all outcomes.
---
...
3. Cycle internals (for diagnosis):
   ...
   - Proposal: `timeout 120 claude -p "<prompt>" > /tmp/harness-proposal-*.diff`.
...
- `timeout 120` is the wall-clock limit for the `claude -p` call. On timeout the
