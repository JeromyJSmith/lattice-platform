---
description: Author TanStack Start routes and server functions for the LATTICE operator UI, enforce client purity (no Anthropic SDK, no @itwin/core-backend), and validate loader coverage.
---
...
- Fixing a client-purity violation (Anthropic SDK or `@itwin/core-backend` import found)
...
4. Scan for client-purity violations:
   ...
   git grep "@itwin/core-backend" -- src/
   ```
...
- Never import Anthropic SDK directly in any `src/` file. Server functions use
  `@tanstack/ai` adapters or the `claude -p` subprocess via the Python sidecar.
- Never import `@itwin/core-backend` in `src/`. Use `@itwin/core-geometry`,
