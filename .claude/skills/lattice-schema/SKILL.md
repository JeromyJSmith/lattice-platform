...
2. Create `pixeltable/migrations/00NN_<topic>.py` using only helper calls:
   ...
   ```
   Never call `pxt.create_table()` directly. Never write `pxt.Geometry` — use
...
5. Also update `meta/ARCHITECTURE.md` and this `CLAUDE.md` LIVE STATE block to
...
- Never write `pxt.Geometry` in any migration file — the CI docs-sync-check blocks it.
...
- All four mandatory rules from the repo `CLAUDE.md` must remain present after any edit.
- The wrong migration path `pixeltable/service/migrations/` does not exist; always use
